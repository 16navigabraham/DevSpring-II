...
export const feePercentage = async () => {
  try {
    const provider = getProvider()
    ...
    const campaigns = await Promise.all(
      campaignAddresses.map(async (address, index) => {
        try {
          ...
          if (code === "0x") {
            console.warn(`Skipping invalid campaign (no contract code): ${address}`)
            return null
          }
          ...
          console.warn(`❌ Error loading campaign ${address}:`, error)
          return null
        }
      })
    )
...
// Get campaign detailed info for withdrawal/refund decisions
export const getCampaignDetails = async (campaignAddress) => {
  try {
    ...
    const now = Math.floor(Date.now() / 1000)
    const isActive = Number(deadline) > now
    const canAutoRefund = (!isGoalMet && !isActive) || (isGoalMet && !withdrawn && Number(withdrawalDeadline) < now)

    return {
      fundingGoal: formatTokenAmount(fundingGoal, 18),
      deadline: new Date(Number(deadline) * 1000),
      totalFundsRaised: formatTokenAmount(totalFundsRaised, 18),
      withdrawalDeadline: new Date(Number(withdrawalDeadline) * 1000),
      owner,
      isGoalMet,
      withdrawn,
