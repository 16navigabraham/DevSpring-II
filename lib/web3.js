    ...
    const campaigns = await Promise.all(
      campaignAddresses.map(async (address, index) => {
        try {
          ...
          // Fallback-safe reads
          const fundingGoal = await contract.fundingGoal().catch(() => 0)
          const deadline = await contract.deadline().catch(() => 0)
          const totalFundsRaised = await contract.getTotalFundsRaised().catch(() => 0)
          const owner = await contract.owner().catch(() => ethers.ZeroAddress)
          const isGoalMet = await contract.isGoalMet().catch(() => false)
          const withdrawn = await contract.withdrawn().catch(() => false)
          const withdrawalDeadline = await contract.getWithdrawalDeadline().catch(() => 0)
          ...
          console.warn(`⚠️ Skipping campaign at ${address}:`, err.message)
          return null
        }
      })
    )
